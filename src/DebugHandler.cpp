        ...
        case osgGA::GUIEventAdapter::KEYDOWN:
            switch (ea.getKey())
            {
                ...
                case osgGA::GUIEventAdapter::KEY_F5:
                    sgg.setDebugging(!sgg.getDebugging());
                    break;

                case osgGA::GUIEventAdapter::KEY_F6:
                    sgbc.getDebugDrawer()->setDrawCotactLifeTime(
                        !sgbc.getDebugDrawer()->getDrawCotactLifeTime());
                    break;

                case osgGA::GUIEventAdapter::KEY_F7:
                    sgbc.getDebugDrawer()->setDrawContactNormal(
                        !sgbc.getDebugDrawer()->getDrawContactNormal());
                    break;

                case osgGA::GUIEventAdapter::KEY_F8:
                {
                    auto seg = osgt::getCameraRay(*view->getCamera(), ea.getX(), ea.getY());
                    auto dir = seg.second - seg.first;
                    dir.normalize();
                    sgbc.shootBox(seg.first, dir);
                    break;
                }

                case osgGA::GUIEventAdapter::KEY_F9:
                    sgg.setPaused(!sgg.getPaused());
                    break;

                case osgGA::GUIEventAdapter::KEY_F10:
                    sgg.setDebugFrames(1);
                    break;

                case osgGA::GUIEventAdapter::KEY_Plus:
                    sgbc.setShootBoxInitialSpeed(sgbc.getShootBoxInitialSpeed() + 10);
                    break;
